...
EODshareReval/
  ...
  run_eod_hidden.vbs                 # Windows: silent VBS wrapper used by the Desktop shortcut
  CreateScheduledTask.ps1            # Windows: create the daily 5 PM scheduled task
  CreateDesktopShortcut.ps1          # Windows: create Desktop shortcut (uses VBS + custom icon)
  GenerateShortcutIcon.ps1           # Windows: regenerate eod_icon.ico
  eod_icon.ico                       # 256x256 PNG-embedded ICO used by the Desktop shortcut
...
Both run the downloader and then open the latest TXT report. The preferred launcher also auto-commits and pushes `asx_eod_output/` to GitHub after each run — skipped when git is missing, when the repo is not on `main`, or when nothing changed; if a push fails (e.g., offline), the commit stays local and rides along with the next run's push. Double-click either from Explorer or run from PowerShell/CMD.

**Desktop shortcut (silent, with custom icon):**

`CreateDesktopShortcut.ps1` creates a Desktop shortcut named **"Run EOD + Open Report"** that:

1. Launches `wscript.exe` against `run_eod_hidden.vbs`, which spawns the downloader **with no visible console window**.
2. Runs `run_eod_downloader_and_report.bat` hidden (Python EOD download + report generation).
3. Auto-commits and pushes `asx_eod_output/` to GitHub (only on `main`; skipped if git is missing or nothing changed).
4. Pops up Notepad++ with the latest `Report_*.txt` once the run finishes.
5. Uses a custom 256x256 stock-chart icon (`eod_icon.ico`, generated by `GenerateShortcutIcon.ps1`).

To create (or recreate) the shortcut:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File CreateDesktopShortcut.ps1
```

To regenerate the icon (e.g., after tweaking colours in the generator script):

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File GenerateShortcutIcon.ps1
```

> Note: because the cmd window is hidden, error `pause` lines in the batch sit invisibly until killed via Task Manager. To debug failures, run `run_eod_downloader_and_report.bat` directly.

### Scheduled task (automatic daily run)

A Windows Task Scheduler job named **"ASX EOD Downloader"** runs every weekday at 5:00 PM (after ASX market close). It launches the same silent VBS wrapper as the desktop shortcut, so it downloads data, generates reports, auto-commits and pushes `asx_eod_output/` to GitHub, then opens the latest report in Notepad++.

- **Schedule:** Monday–Friday at 5:00 PM
- **Action:** `wscript.exe "run_eod_hidden.vbs"` (same flow as the **"Run EOD + Open Report"** shortcut)
