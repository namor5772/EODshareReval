...
### Windows one-click (batch + shortcut)

On Windows, you can use the included batch file to run the downloader and automatically open the newest TXT report in Notepad++ (or Notepad if Notepad++ is unavailable):

- Double-click `RunEODAndOpenReport.bat` (repo root), or run it from a terminal.
- The script will:
  - Execute `asx_eod_downloader.py` using `py` (Python launcher) or `python` as a fallback.
  - Locate the most recent `asx_eod_output\Report_TXT\Report_*.txt` file.
  - Open it in Notepad++ if found on PATH or in Program Files; otherwise fall back to Notepad.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File CreateDesktopShortcut.ps1 -ShortcutName "EOD Report.lnk"
```
